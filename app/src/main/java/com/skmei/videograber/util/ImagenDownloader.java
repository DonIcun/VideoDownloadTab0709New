...
public class ImagenDownloader {
	...
	public ImagenDownloader(Resources pResources, Bitmap pDefaultBitmap) {
		mResources = pResources;
		mBitmap = pDefaultBitmap;
	}
	...
	private void forceDownload(String url, ImageView imageView, String cookie) {
		...
		if (cancelPotentialDownload(url, imageView)) {
			...
			DownloadedDrawable downloadedDrawable = new DownloadedDrawable(task, mResources, mBitmap);
			...
			try {
				...
				if (entity != null) {
					...
					try {
						...
						//final Bitmap bitmap = BitmapFactory.decodeFile(mUri.getPath(),options);

						final Bitmap bitmap = BitmapFactory.decodeByteArray(data, 0, data.length,options);

						//final Bitmap bitmap = Bitmap.createScaledBitmap(dataStream, 50 , 50 , true);
	...
	static class DownloadedDrawable extends BitmapDrawable {
		...
		public DownloadedDrawable(BitmapDownloaderTask bitmapDownloaderTask, Resources mResources, Bitmap mBitmap) {
